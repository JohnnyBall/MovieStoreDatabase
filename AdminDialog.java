...
{  
   ...
   {  
      System.out.println("ATTEMPTING TOPTEN");
       tmp = " SELECT rrr.rid,r.title FROM rentals_record_rents rrr, rentals r WHERE (rrr.from_date between (CURDATE() - INTERVAL 30 DAY) AND CURDATE()) AND rrr.rid = r.rid GROUP BY rrr.rid ORDER BY count(*) desc LIMIT 10;";
       this.doQuery(tmp);
   }//END OF SEARCH ELSE
   else if(e.getActionCommand().equals("HOURS"))
   {
       System.out.println("ATTEMPTING HOURS");
       tmp = " SELECT DISTINCT r.rid, r.title, rrr.trackingnum, a.street, a.city, a.state, a.zip,a.phone, rrr.from_dateFROM rentals_record_rents rrr, rentals r, user u, person p, address a, has_address ha WHERE rrr.from_date between (now() - INTERVAL 1 DAY) AND NOW()  AND rrr.rid = r.rid AND rrr.pid = u.pid AND u.pid = p.pid AND p.pid = ha.pid AND ha.zip = a.zip AND ha.street = a.street;";
       this.doQuery(tmp);
   }//END OF SEARCH ELSE
}
void doQuery(String query)
{
   ...
   System.out.println("MADE IT TO DdoQuery");
    try 
      {
         statement = connection.createStatement();
         resultSet = statement.executeQuery(query);
         System.out.println("MADE IT TO THE TRYCATCJ");
         ...
         {
            System.out.println("MADE IT TO THE Telseelseelseelseelseelseelseelseelseelse");
